...
async def lifespan(app):
    ...
    if vector_store.documents:
        ...
        log.info("Indexed existing corpus on startup", doc_count=len(vector_store.documents))
    ...
    if config["reranker"]["enabled"] and cohere_key:
        ...
        log.warning("Cohere API key not found or reranker disabled. Reranking will be skipped.")
        reranker = None
...
def create_app():
    ...
    def query(p: dict):
        ...
        messages = [
            SystemMessage(content="You are a helpful enterprise assistant. Answer the user's question using only the provided context. If you don't know the answer or the context doesn't contain it, say you don't know."),
